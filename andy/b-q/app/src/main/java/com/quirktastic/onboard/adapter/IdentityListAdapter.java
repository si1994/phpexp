...
public class IdentityListAdapter extends RecyclerView.Adapter<IdentityListAdapter.InterestListHolder> {


    private ArrayList<IdentityListItem> genderList;
    private Context context;

    public IdentityListAdapter(Context context, ArrayList<IdentityListItem> list) {
    ...
    public void onBindViewHolder(@NonNull InterestListHolder holder, final int position) {
        ...
        if (genderList.get(position).getIdentityName() != null) {
            holder.tvInterestItem.setText(genderList.get(position).getIdentityName());
        }
        ...
        holder.tvInterestItem.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                manageSelected(position);
            }
        });
    }

    private void manageSelected(int position) {
        for (int i = 0; i < genderList.size(); i++) {
            if (i == position) {
                genderList.get(i).setSelected(true);
            } else  {
                genderList.get(i).setSelected(false);
            }
        }
